...
// Uses brand palette so it reads as real imagery at-a-glance.
...
function ModelSVG({ seed = 'a', label, ratio = '3/4' }) {
  ...
  return (
    <div style={{ position: 'relative', aspectRatio: ratio, overflow: 'hidden', background: p.bg }}>
      <svg viewBox="0 0 300 400" preserveAspectRatio="xMidYMid slice" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%' }}>
        ...
        {/* Soft architectural elements — arch / wall hint */}
        {variant === 0 && (
          <g opacity="0.18">
        ...
        <g>
          ...
          {/* Subtle gold trim along kameez bottom */}
      ...
      {label && (
        <div style={{
          ...
          color: 'rgba(255,255,255,0.7)', mixBlendMode: 'difference',
        }}>{label}</div>
      )}
    </div>
  );
...
function FlatLaySVG({ seed = 'b', label, ratio = '3/4' }) {
  const p = PALETTES[hash(seed) % PALETTES.length];
  const sid = idSafe(seed);
...
function TextureSVG({ seed = 'c', label, ratio = '3/4' }) {
  const p = PALETTES[hash(seed) % PALETTES.length];
  const sid = idSafe(seed);
  return (
    <div style={{ position: 'relative', aspectRatio: ratio, overflow: 'hidden', background: p.fg }}>
      <svg viewBox="0 0 300 400" preserveAspectRatio="xMidYMid slice" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%' }}>
        <defs>
...
function SmartImage({ seed, label, ratio = '3/4', kind, style, className = '' }) {
  let K = kind;
  if (!K) {
...
Object.assign(window, { ModelSVG, FlatLaySVG, TextureSVG, SmartImage });
